...
const logPlaceHolder = {
	...
	initLogPlaceHolder: (req, controllarName, callerName = null) => {
		...
		logger.debug('fields =>', req.fields);
...
const baseFilter = async (reqBody, Model, localWhere) => {
	...
	if (reqBody.search_term) {
		if (localWhere.$or || reqBody.$or) {
			where.$or = {};
			if (reqBody.$or) {
				let mor = JSON.parse(reqBody.$or);
				mor.map(element => {
		...
		if (localWhere.$and || reqBody.$and) {
			where.$and = {};
			if (reqBody.$and) {
				let mand = JSON.parse(reqBody.$and);
				mand.map(element => {
